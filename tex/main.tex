...
The bijective function $\zeta$ maps two sets $\zeta:\;A \to B$, where $A$ is the set of all atomic literals $\phi \in A$ where $\phi$ is an atomic literal used in $w$, and $B$ is the set of output atomic formulas $a_{j}$ where $j \in [1,\,|A|]$. One property of $\zeta$ is that the mapping need not to be linear, i.e., $\phi_1 \in A$ does not necessarily have to map to $a_1 \in B$; as long as the bijective property holds, any mapping is valid. In following examples, we assume that $A$ is finite, but for languages which allows a countably infinite number of atomic literals, e.g., $A = \{s_1,\,s_2,\,s_3, ...\}$, a solution is to map $A$ to a one-to-one set of positive integers (i.e., $s_n \in A \mapsto a_n \in B$ where $n \in \mathbb{Z}^+$). A slight alteration of this definition will also work if the literals in $A$ do not correspond proportionally to the positive integers.
